...
async function viewOrderBill(orderId) {
    try {
        ...
        const labels = [
            ...
            ['Pickup Scheduled', bill.pickup_at ? formatPickupTime(bill.pickup_at) : null],
        ...
        card.innerHTML = `
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div class="min-w-0">
                    ...
                    <p class="text-[11px] text-slate-400 mt-2"><i class="fa-regular fa-clock mr-1"></i>Ordered: ${formatOrderDateTime(order.created_at)}</p>${order.pickup_at ? `<p class="text-[11px] text-teal-700 font-bold mt-1"><i class="fa-solid fa-person-walking mr-1"></i>Pickup: ${formatPickupTime(order.pickup_at)}</p>` : ""}
                </div>
