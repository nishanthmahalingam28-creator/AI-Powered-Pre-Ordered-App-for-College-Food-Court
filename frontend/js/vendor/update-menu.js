...
async function removeItem(id,name) { if(!confirm(`Remove "${name}" from the menu?`))return; const res=await fetch(`${UPDATE_MENU_API}/vendor/menu/item/${id}`,{method:'DELETE',credentials:'include'}); const data=await res.json(); if(data.success) await loadUpdateMenu(); else alert(data.message||'Delete failed.'); }
